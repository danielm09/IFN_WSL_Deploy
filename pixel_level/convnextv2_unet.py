...
class ConvNeXtV2_unet(nn.Module):
        ...
        if self.use_orig_stem:
            self.stem_orig = nn.Sequential(
                nn.Conv2d(
                    in_chans,
                    dims[0],
                    kernel_size=3,#patch_size // (2 ** (self.num_stage - 1)),
                    stride=1, padding=1#patch_size // (2 ** (self.num_stage - 1)),
                ),
            ...
            self.stem = nn.Sequential(
                nn.Conv2d(
                    dims[0],
                    dims[0],
                    kernel_size=2,#patch_size // (2 ** (self.num_stage - 1)),
                    stride=2,#patch_size // (2 ** (self.num_stage - 1)),
                    groups=dims[0],
                ),
                LayerNorm(dims[0], eps=1e-6, data_format="channels_first"),
            )
            ...
            if i == 3:
                ...
                self.upsample_layers.append(
                    UpsampleBlock(
                        dims[i] * 2,
                        int(dims[i]),
                        scale_factor=2,#patch_size // (2 ** (self.num_stage - 1)),
                    )
                )
    ...
    def encoder(self, x: Tensor) -> Tuple[Tensor, List[Tensor]]:
        ...
        if self.use_orig_stem:
            ...
            # self.tmp_var = x
            enc_features.append(x)
        #print('initial conv and stem', x.shape)
    ...
    def decoder(self, x: Tensor, enc_features: List[Tensor]):
        ...
        if not self.use_orig_stem:#self.args.use_orig_stem:
            tmp = enc_features.pop()
            x = torch.cat([x, tmp], dim=1)
